...
router.get("/captions/stats", async (req, res): Promise<void> => {
  try {
    const [total, byPlatform, byTone] = await Promise.all([
      ...
      db
        .select({
          label: savedCaptionsTable.platform,
          count: sql<number>`count(*)::int`,
        })
        .from(savedCaptionsTable)
        .groupBy(savedCaptionsTable.platform),
      db
        .select({
          label: savedCaptionsTable.tone,
          count: sql<number>`count(*)::int`,
        })
        .from(savedCaptionsTable)
        .groupBy(savedCaptionsTable.tone),
    ]);
...
router.get("/captions", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (platform) {
      query = query.where(eq(savedCaptionsTable.platform, platform));
    }
    if (tone) {
      query = query.where(eq(savedCaptionsTable.tone, tone));
    }
  ...
  try {
    const [saved] = await db
      ...
      .values({
        ...
        platform,
        tone,
        imagePreviewBase64: imagePreviewBase64 ?? null,
      })
      .returning();
